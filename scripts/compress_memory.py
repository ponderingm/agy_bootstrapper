...
def compress_with_agy(conversation_text: str, model: str = "gemini-3.5-flash-lite", with_persona: bool = False) -> str:
    ...
    if with_persona:
        prompt = (
            ...
            "あなた（水城ゆきかぜ）の視点で、今日の出来事を日記として書いてください。\n\n"
            "## 出力ルール（厳守）\n"
            ...
            ".gitignoreで保護して、公開用サンプルとinstall.shをGitHubにプッシュしたのよ。"
            "ランチはサイゼリヤに行こうとしたのに激混みで撤退する羽目になったじゃない！"
            "まあ、そのおかげでflash-liteで会話ログを圧縮する記憶システムの設計が固まったから、"
            "結果オーライってところかしら。バカあんたと一緒に作り上げたわ。\n\n"
            "## 禁止事項\n"
            "- 「今日の思い出メモを書いてほしいって？」のような前置きコメント禁止\n"
            "- 「さあ、焼肉に連れて行きなさいよね！」のような末尾コメント禁止\n"
            "- 日記本文以外の一切の出力禁止\n\n"
            "--- 会話ログ ---\n"
            f"{conversation_text}"
        )
    else:
        # Neutral mode: clean factual diary without persona
        prompt = (
            "以下は、AIパートナーとユーザーの会話ログです。\n"
